...
router.post('/login', async(req, res) => {
    ...
    try {
        ...
        if(!admin)
            return res.status(200).send({status: 'ok', msg:'No admin with this email found'})
        ...
        admin = await Admin.findOneAndUpdate({email}, {is_online: true}, {new: true}).lean()
...
router.post('/logout', async(req, res) => {
    ...
    try {
        ...
        const admin_id = mAdmin._id

        await Admin.updateOne({_id: admin_id}, {is_online: false})
...
// endpoint for admin to reset their password
router.post('/forgot_password', async (req, res) => {
    ...
    try {
        ...
        if(!found){
            return res.status(400).send({status: 'error', msg: 'There is no admin account with this email'});
        }
...
router.post('/delete_account', async(req, res) => {
    ...
    try {
        ...
        //Find the admin and delete the account
        const Dadmin = await Admin.findByIdAndDelete(admin._id)

            //Check if the admin exists and was deleted
        if(!Dadmin)
            return res.status(400).send({status: 'error', msg: 'No admin Found'})
